...
const PHANTOMUnits = 100000000

prompt.message = chalk.blue.bold('PHANTOM-JavaScript-CLI')
...
class Input {
  ...
  async amountToXPH (amount) {
    try {
      ...
      let value = parseInt(amount * PHANTOMUnits * exchangeRate, 10)

      // check for overflow on the PHANTOMToshis and inputted amount
  ...
  async __getXPHTicker (currency) {
    currency = currency.toLowerCase().trim()
    let url = 'https://api.coinmarketcap.com/v1/ticker/phantom/'
