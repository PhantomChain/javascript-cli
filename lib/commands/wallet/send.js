...
const PHANTOMToshi = Math.pow(10, 8)
const schema = {
...
module.exports = async (amount, recepient, cmd) => {
  ...
  try {
    ...
    // Convert the amount to integer in XPH*100000000
    amount = await input.amountToXPH(amount)
    ...
    if (interactive) {
      ...
      const value = parseFloat(amount) / PHANTOMToshi
